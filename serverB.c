/*
** serverB.c -- serverB, one of the backend servers to process the data.
*/
...
int main(void)
{
	...
	if ((rv = getaddrinfo(NULL, BUDPPORT, &hints, &servinfo)) != 0) {//from Beej's Guide
	...
	if (p == NULL) {
		fprintf(stderr, "B: failed to bind socket\n");
		return 2;
	}
	...
	if ((numbytes = recvfrom(sockfd, buf, MAXBUFLEN, 0,
		(struct sockaddr *)&their_addr, &addr_len)) == -1) {//from Beej's Guide
		perror("recvfrom");
		exit(1);
	}
	...
	if ((rv = getaddrinfo(inet_ntop(their_addr.ss_family,
			...
			s, sizeof s), DUDPPORT, &hints, &servinfo)) != 0) {
